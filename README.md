...
- M5Stack NanoC6（ESP32-C6）→ 2026-08-30に1台購入、配送待ち。オンボードのセラミックアンテナの性能とLevel 2（送受信ペア構成）が問題なく動作するかを検証予定（詳細は`docs/handoff.md`）
...
- ESP32-S3の送受信ペア構成（Level 2）は未検証のため、まずは自分でトラフィックを生成して自分で受信する「セルフセンシング（Level 1）」構成で検証している
