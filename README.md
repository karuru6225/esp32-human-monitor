...
- M5Stack NanoC6（ESP32-C6）→ **採用断念（2026-08-31）**。PlatformIOの標準ビルド環境がESP32-C6のArduinoフレームワークに未対応で、回避策も環境破壊のリスクが高く見合わないと判断（詳細は`docs/handoff.md`）。SpecificDeviceCSI（専用デバイスCSI方式）用にはM5Stack ATOM Liteを3台追加購入する方針に変更（`env:m5atom-lite`でビルド実績あり）
...
- ESP32-S3の送受信ペア構成（SpecificDeviceCSI、専用デバイスCSI方式）は未検証のため、まずは自分でトラフィックを生成して自分で受信する「RouterCSI（ルーターCSI方式）」構成で検証している
- 複数ノードでの精度向上（ゾーン分離・多層融合）は、RouterCSI/SpecificDeviceCSIの選択とは別軸の話で、独立したノードを増やして結果を融合することが本質
- 将来ペア構成（TX専用+RX専用）でしっかり作り込みたくなったら、公式のCSI性能ランキング（C5>C6>C3≈S3>ESP32）上位のESP32-C6/C5系ボードが候補
